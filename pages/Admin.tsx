...
export const AdminDashboard = () => {
  ...
  React.useEffect(() => {
    console.log('AdminDashboard effect:', { activeTab, isAdmin: user?.isAdmin, token: localStorage.getItem('qt_token') });
  ...
  const handleAddProduct = (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.price || !formData.imageUrl) {
      showNotification("Por favor completa nombre, precio e imagen", 'error');
      return;
    }
    ...
    showNotification('Producto agregado exitosamente al inventario.', 'success');
  };
  ...
  const handleUpdateAppointment = (id: string) => {
    ...
    setApptDate('');
    setApptTime('');
    showNotification('Cita reprogramada', 'success');
  };
  ...
  const renderOverview = () => {
    if (statsError) {
      const isForbidden = statsError.includes('403');
      return (
        <div className="p-8 text-center">
          <p className="text-red-500 font-bold mb-4">
            {isForbidden ? 'Tu sesión ha expirado o no tienes permisos.' : `Error cargando estadísticas: ${statsError}`}
          </p>
          {isForbidden && (
            <p className="text-gray-500 mb-4 text-sm">Por favor inicia sesión nuevamente para actualizar tus credenciales.</p>
          )}
          <div className="flex gap-4 justify-center">
            <button onClick={() => window.location.reload()} className="bg-black text-white px-4 py-2 rounded">Reintentar</button>
            {isForbidden && (
              <button onClick={() => { localStorage.clear(); window.location.href = '/login'; }} className="border border-black px-4 py-2 rounded">Cerrar Sesión</button>
            )}
          </div>
        </div>
      );
    }
    if (loadingStats || !stats) return <div className="p-8 text-center text-gray-500">Cargando estadísticas...</div>;
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-bold text-black">${(stats.revenue || 0).toLocaleString()}</h3>
              <p className="text-gray-500 text-xs uppercase font-bold">Ingresos Totales</p>
            </div>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-center gap-4">
            <div className="bg-blue-100 p-3 rounded-full text-blue-600"><Package size={24} /></div>
            <div>
              <h3 className="text-2xl font-bold text-black">{stats.orders}</h3>
              <p className="text-gray-500 text-xs uppercase font-bold">Total Pedidos</p>
            </div>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs uppercase font-bold">Usuarios Registrados</p>
            </div>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex items-center gap-4">
            <div className="bg-red-100 p-3 rounded-full text-red-600"><Box size={24} /></div>
            <div>
              <h3 className="text-2xl font-bold text-black">{stats.lowStock}</h3>
              <p className="text-gray-500 text-xs uppercase font-bold">Alertas Stock Bajo</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-96">
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
            <h3 className="font-bold uppercase text-sm mb-4 text-black">Tendencia de Ventas (Últimos 7 días)</h3>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.salesChart}>
                ...
                <RechartsTooltip />
                <Bar dataKey="daily_revenue" name="Ventas ($)" fill="#000000" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex flex-col">
            <h3 className="font-bold uppercase text-sm mb-4 text-black">Distribución por Categoría</h3>
          ...
          <tbody>
            {orders.map(order => (
              <tr key={order.id} className="border-b">
                ...
                <td className="p-3">
                  <span className={`px-2 py-1 rounded text-xs uppercase font-bold ${order.status === 'delivered' ? 'bg-blue-100 text-blue-800' :
