...
import { Check, Shield, TrendingUp, DollarSign, ArrowRight, User as UserIcon, Building, MapPin, DollarSign as MoneyIcon, Send, Clock, Package, MessageCircle, FileText, Download, LogOut, PlusCircle, Truck, Calendar, AlertTriangle } from 'lucide-react';
            ...
            {step === 2 && (
                <div className="animate-in fade-in slide-in-from-right duration-500">
                    ...
                    <div className="flex justify-between gap-4">
                        ...
                        <button 
                            onClick={() => {
                                if(!formData.businessName || !formData.phone) return alert('Por favor completa los campos requeridos');
                                setStep(3);
                            }} 
...
const DistributorDashboard = () => {
    ...
    const handleQuickAdd = (product: any) => {
        ...
        if (qty > 0) {
            ...
            alert(`Añadido ${qty} unidades de ${product.name} al carrito`);
    ...
    const handleResign = (e: React.FormEvent) => {
        ...
        if (confirm('¿Estás seguro de renunciar? Perderás tus descuentos inmediatamente.')) {
            ...
            alert('Has renunciado a tu distribución. Volverás al inicio.');
            navigate('/');
        }
    };
                ...
                <div className="flex-1 bg-white border border-gray-200 rounded-lg p-4 md:p-8 min-h-[500px] shadow-sm">
                    ...
                    {activeTab === 'support' && (
                        <div>
                            ...
                            <form className="max-w-lg mb-8" onSubmit={(e) => { e.preventDefault(); createTicket(pqrsSubject, pqrsMsg); alert('Ticket creado'); setPqrsMsg(''); }}>
                                <div className="mb-4">
...
export const Distributors = () => {
    ...
    // 2. If Applying -> Form
    if (isApplying) {
        return <ApplicationForm onCancel={() => setIsApplying(false)} />;
    }
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="bg-black text-white py-20 px-4 text-center">
                <h1 className="text-4xl md:text-6xl font-bold uppercase mb-4 tracking-widest">Nuestros Distribuidores</h1>
                <p className="max-w-2xl mx-auto text-gray-400 mb-8">Encuentra los productos Queen Touch cerca de ti o únete a nuestra red de distribución oficial.</p>
                
                {appStatus === 'pending' ? (
                    <div className="bg-yellow-100 text-yellow-800 p-4 rounded inline-block">
                        <p className="font-bold flex items-center gap-2"><Clock size={20}/> Tu solicitud está en revisión</p>
                    </div>
                ) : appStatus === 'rejected' ? (
                     <div className="bg-red-100 text-red-800 p-4 rounded inline-block">
                        <p className="font-bold">Tu solicitud fue rechazada. Contacta a soporte.</p>
                    </div>
                ) : (
                    <button 
                        onClick={() => user ? setIsApplying(true) : navigate('/login')}
                        className="bg-white text-black px-8 py-3 uppercase font-bold hover:bg-gray-200 transition"
                    >
            ...
            <div className="container mx-auto px-4 py-12">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                    {distributors.map((d, i) => (
                        <div key={i} className="bg-white p-6 rounded shadow border border-gray-100">
                             <h3 className="text-xl font-bold uppercase mb-4 flex items-center gap-2">
                                <MapPin size={24}/> {d.city}
                             </h3>
                             <ul className="space-y-2">
                                {d.list.map((shop, j) => (
                                    <li key={j} className="flex items-center gap-2 text-gray-600">
                                        <Building size={16} /> {shop}
                                    </li>
                                ))}
                             </ul>
                        </div>
