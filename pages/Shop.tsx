...
import { ShoppingCart, PlayCircle, ChevronRight, ArrowLeft, AlertCircle, Heart, Tag } from 'lucide-react';
...
const ProductList = ({ categoryId, subcategoryId }: { categoryId: string, subcategoryId: string }) => {
  const { products, isInWishlist, toggleWishlist, user, getWholesalePrice } = useStore();
          ...
          return (
            <div key={p.id} className="block group relative">
              ...
              <Link to={`/product/${p.id}`}>
                ...
                {isDistributor ? (
                  <div className="mt-1">
                    <span className="text-gray-400 text-xs line-through block">${p.price.toLocaleString()}</span>
                    <span className="text-purple-700 font-bold text-sm block">${wholesalePrice.toLocaleString()} (Mayorista)</span>
                  </div>
                ) : (
...
const ProductDetail = ({ productId }: { productId: string }) => {
  const { products, addToCart, isInWishlist, toggleWishlist, user, getWholesalePrice } = useStore();
  ...
  const wholesalePrice = getWholesalePrice(product.price);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div className="flex flex-col">
          ...
          {isDistributor ? (
            <div className="mb-2 p-3 bg-purple-50 border border-purple-100 rounded">
              <p className="text-gray-500 text-sm line-through">Precio Público: ${product.price.toLocaleString()}</p>
              <p className="text-2xl text-purple-700 font-bold">${wholesalePrice.toLocaleString()}</p>
              <p className="text-xs text-purple-600 font-bold uppercase flex items-center gap-1"><Tag size={12} /> Precio Mayorista Aplicado</p>
            </div>
          ) : (
...
export const Shop = () => {
  ...
  return (
    <>
      {/* Distributor Banner */}
      {isDistributor && (
        <div className="bg-purple-700 text-white text-center py-2 text-xs font-bold uppercase tracking-widest sticky top-20 z-40">
          Modo Mayorista Activo - Precios Especiales
        </div>
      )}
      ...
      {window.location.hash.includes('/product/') ? (
        ...
        <ProductList categoryId={params.category!} subcategoryId={params.subcategory} />
      ) : params.category ? (
        <SubcategoryList categoryId={params.category} />
      ) : (
        <CategoryList />
      )}
    </>
